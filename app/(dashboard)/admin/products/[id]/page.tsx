...
const DashboardProductDetails = ({
      ...
      .then((response) => {
        if (response.status !== 204) {
          if (response.status === 400) {
            ...
            throw Error("Đã xảy ra lỗi khi xóa sản phẩm");
          }
        } else {
  ...
  const updateProduct = async () => {
    ...
    fetch(`http://localhost:3001/api/products/${id}`, requestOptions)
      ...
      .then((data) => toast.success("Cập nhật sản phẩm thành công"))
      .catch((error) => {
    ...
    <div className="bg-white flex justify-start max-w-screen-2xl mx-auto xl:h-full max-xl:flex-col max-xl:gap-y-5">
      ...
      <div className="flex flex-col gap-y-7 xl:ml-5 w-full max-xl:px-5">
        ...
        <div>
          <label className="form-control w-full max-w-xs">
            <div className="label">
              <span className="label-text">Sản phẩm có sẵn không?</span>
            </div>
            <select
        ...
        <p className="text-xl max-sm:text-lg text-error">
          Để xóa sản phẩm, trước tiên bạn cần xóa tất cả các bản ghi của nó trong đơn hàng (bảng customer_order_product)
        </p>
      </div>
    </div>
  );
