...
const CheckoutPage = () => {
  ...
  const makePurchase = async () => {
    if (
      ...
      if (!isValidNameOrLastname(checkoutForm.name)) {
        toast.error("Bạn nhập tên không hợp lệ");
        return;
      }

      if (!isValidNameOrLastname(checkoutForm.lastname)) {
        toast.error("Bạn nhập họ không hợp lệ");
        return;
      }

      if (!isValidEmailAddressFormat(checkoutForm.email)) {
        toast.error("Bạn nhập email không hợp lệ");
        return;
      }

      if (!isValidNameOrLastname(checkoutForm.cardName)) {
        toast.error("Bạn nhập tên trên thẻ không hợp lệ");
        return;
      }

      if (!isValidCardNumber(checkoutForm.cardNumber)) {
        toast.error("bạn nhập số thẻ không hợp lệ");
        return;
      }

      if (!isValidCreditCardExpirationDate(checkoutForm.expirationDate)) {
        toast.error(
          "Bạn nhập ngày hết hạn thẻ tín dụng không hợp lệ"
        );
        return;
      }

      if (!isValidCreditCardCVVOrCVC(checkoutForm.cvc)) {
        toast.error("Bạn nhập mã CVC hoặc CVV không hợp lệ");
        return;
      }
      ...
      toast.error("Bạn cần phải điền tất cả thông tin vào các ô bên dưới");
    }
  };
  ...
  return (
    <div className="bg-white">
      <SectionTitle title="Tiến hành thanh toán" path="Trang chủ | Giỏ hàng | Thanh toán" />
          ...
          <div className="mx-auto max-w-lg lg:max-w-none">
            ...
            <dl className="hidden space-y-6 border-t border-gray-200 pt-6 text-sm font-medium text-gray-900 lg:block">
              <div className="flex items-center justify-between">
                <dt className="text-gray-600">Tổng giá trị sản phẩm</dt>
                <dd>${total}</dd>
              </div>
